...
var totalChoc = n / c;
        ...
        while (m <= totalWrappers) {
            var wrapperTogive=totalWrappers-(totalWrappers%m) ;

            freeChocs = Math.floor(totalWrappers / m);

            totalWrappers = Math.floor((totalWrappers - wrapperTogive) + freeChocs);

            totalChoc=totalChoc + freeChocs;
        }
    return Math.floor(totalChoc);
}
